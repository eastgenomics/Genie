...
def calculate_count_for_all_haemonc_cancers(genie_data, haemonc_cancer_types):
    """
    Count each unique variant in haemonc cancers.
    ...
    haemonc_cancer_types : list
        List of haemonc cancer types to subset to
    variant_columns : list
        Columns defining a unique variant
    ...
    pd.DataFrame
        Count data for each variant in haemonc cancers
    """
    # Subset to haemonc cancer types
    subset = genie_data[
        genie_data["CANCER_TYPE"].isin(haemonc_cancer_types)
    ].copy()
    # Count unique patients per variant
    counts = (
        subset.drop_duplicates(subset=["PATIENT_ID", "grch38_description"])
        .groupby("grch38_description")["PATIENT_ID"]
        .nunique()
        .rename("exact_haemonc_cancers_count")
        .reset_index()
    )
    # Ensure all variants are present, fill missing with zero
    all_variants = genie_data.drop_duplicates(subset="grch38_description")[
    ...
    result["exact_haemonc_cancers_count"] = result[
        "exact_haemonc_cancers_count"
    ].astype(int)
    return result
